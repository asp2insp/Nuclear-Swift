...
class StoreTests: XCTestCase {
    ...
    class ExperimentStore : Store {
        ...
        override func initialize() {
            ...
            self.on("removeExperiment", handler: {(state, payload, action) in
                var newState = state
                var experiments = Immutable.getIn(newState, ["experiments"] as [SimpleDep]) as! [Immutable.State]
    ...
    func testHandlesAddExperiments() {
        ...
        let results = Immutable.getIn(newState, ["experiments"] as [SimpleDep]) as! [Immutable.State]
